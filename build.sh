...
if !command
  - v zip > / dev / null 2 > &1;
then echo "zip is not installed. Please install zip to "
          "proceed." exit 1 fi if !command -
        v clang >
    / dev / null 2 > &1;
then echo "clang is not installed. Please install clang to proceed." exit 1 fi

    mydir = $(dirname "$(realpath " $0 ")") version =
        $(grep version = "$mydir" / src / R / module.prop |
                         awk - F '=' '{print $2}') if[!-d "$mydir" / output];
then mkdir "$mydir" / output fi

                          clang -
    O3 "$mydir" / src / source.c -
    o "$mydir" / src /
        main

        if[-d "$mydir" / NR];
then rm - rf "$mydir" / NR fi mkdir -
        p "$mydir" / NR / Priority cp "$mydir" / src / gamelist.txt "$mydir" /
            NR / Priority cp "$mydir" / src / NR / run.sh "$mydir" / NR /
            Priority cp "$mydir" / src / source.c "$mydir" / NR /
            Priority cp "$mydir" / src / toast.apk "$mydir" / NR /
            Priority cp "$mydir" / src / main "$mydir" / NR /
            Priority cd "$mydir" / NR ||
    exit zip - r./ "AllGame Priority $version NR.zip"./*
cd "$mydir" || exit
